...
exports.delete = async(req, res) => {
    try {
        ...
        await pool.query(`UPDATE categories SET is_deleted = true  where "id" = '${id}'`);
...
exports.getCategoryDropDown = async(req, res) => {
    try {
        const response = await pool.query(`select id, name, code FROM categories where is_deleted = false and is_active = true `);
...
exports.changeStatus = async(req, res) => {
    try {
        ...
        await pool.query(`UPDATE categories SET is_active = ${status} where "id" = '${id}'`);
        res.status(STATUS_CODE.SUCCESS).send();
    } catch (error) {
        res.status(STATUS_CODE.ERROR).send({
            message: error.message || MESSAGES.COMMON.ERROR
