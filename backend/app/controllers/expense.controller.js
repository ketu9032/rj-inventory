...
exports.delete = async(req, res) => {
    try {
        ...
        await pool.query(
            `UPDATE expense SET is_deleted = true  where "id" = '${id}'`
        );
...
exports.update = async(req, res) => {
    try {
        ...
        await pool.query(
            `UPDATE expense SET user_id='${userId}', description='${description}' , amount='${amount}', date='${date}' where id = ${id};
       `
        );
...
exports.changeStatus = async(req, res) => {
    try {
        ...
        await pool.query(`UPDATE expense SET is_active = ${status} where "id" = '${id}'`);
        res.status(STATUS_CODE.SUCCESS).send();
    } catch (error) {
        res.status(STATUS_CODE.ERROR).send({
            message: error.message || MESSAGES.COMMON.ERROR
